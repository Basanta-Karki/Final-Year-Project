...
function TutorForm() {
  ...
  async function registerAsTutor() {
    ...
    try {
      ...
      const resp = await axios.post("http://localhost:5001/api/user/setRole", {
        ...
        userId: "dkjfhdkj",  // Replace with the actual user ID
  ...
  const handleAddSubject = (e) => {
    e.preventDefault();
    if (!formData.Subjects.includes(formData.CurrentSubject) && formData.CurrentSubject !== "") {
      setFormData(prev => ({
        ...prev,
        Subjects: [...prev.Subjects, formData.CurrentSubject],
        CurrentSubject: ""
      }));
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-6">Select Teaching Mode</h2>
        <select
          name="TeachingMode"
          id="teachingMode"
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-6">Document Verification</h2>
        ...
        <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded hover:bg-blue-700">Submit Application</button>
      </form>
    </div>
  );
}
